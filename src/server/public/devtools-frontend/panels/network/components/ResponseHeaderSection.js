...
export class ResponseHeaderSection extends HTMLElement {
    ...
    #updateOverrides(headerName, headerValue, index) {
        ...
        const headersToUpdate = this.#headerEditors.filter(header => header.name === headerName && (header.value !== header.originalValue || header.isOverride));
        const rawPath = Persistence.NetworkPersistenceManager.NetworkPersistenceManager.instance().rawPathFromUrl(this.#request.url(), true);
    ...
    #render() {
        ...
        const headerDescriptors = this.#headerEditors.map((headerEditor, index) => ({ ...this.#headerDetails[index], ...headerEditor }));
      ...
      ${headerDescriptors.map((header, index) => html `
        <${HeaderSectionRow.litTagName} .data=${{
            header: header,
        }} @headeredited=${this.#onHeaderEdited} data-index=${index}></${HeaderSectionRow.litTagName}>
      `)}
