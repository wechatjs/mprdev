...
export async function resolveFrameFunctionName(frame) {
    ...
    const sourceMap = Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().sourceMapForScript(frame.script);
    if (!sourceMap) {
        return null;
    }
    ...
    const openRange = new TextUtils.TextRange.TextRange(startLocation.lineNumber, startLocation.columnNumber, startLocation.lineNumber, startLocation.columnNumber + 1);
